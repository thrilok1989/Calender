...
📉 <b>Signal Conditions Met:</b>
• CE OI Change: {signal_data['ce_oi_change']:,}
• PE OI Change: {signal_data['pe_oi_change']:,}
• CE/PE OI Ratio: {abs(signal_data['ce_oi_change']/signal_data['pe_oi_change']) if signal_data['pe_oi_change'] != 0 else 'inf':.2f}x

🎯 <b>All ATM Biases are Bearish:</b>
• ChgOI Bias = Bearish
• Volume Bias = Bearish
• AskQty Bias = Bearish
• BidQty Bias = Bearish  
...
• Bid-Ask Pressure: {signal_data['pressure']:,}
...
⏰ Time: {datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d %H:%M:%S")}
        ...
        st.session_state.signal_history.append({
            ...
            'entry': pe_ltp,
            'stop_loss': stop_loss,
            'ce_oi_change': signal_data['ce_oi_change'],
            'pe_oi_change': signal_data['pe_oi_change']
