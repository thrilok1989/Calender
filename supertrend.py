...
def create_chart(df, title, bins=20, volume_profile_timeframe="1W"):
    if df.empty:
        return go.Figure()
    fig = make_subplots(
    ...
    # Resample for volume profile calculation
    ohlc = df.resample(volume_profile_timeframe, on='datetime').agg({
        "open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum"
    }).dropna()
    
    # Add volume profile and POC for each resampled bar
    for idx, row_ohlc in ohlc.iterrows():
        ...
        mask = (df['datetime'] >= idx) & (df['datetime'] < idx + pd.Timedelta(volume_profile_timeframe))
        sub_df = df.loc[mask]
        ...
        # Add volume profile bars as negative width bars on left side of timeframe
        fig.add_trace(go.Bar(
            x=-hist_scaled,
            y=bin_centers,
            base=idx,
            ...
            name=f"Vol Profile {idx.strftime('%Y-%m-%d')}",
            showlegend=False,
            opacity=0.5
        ), row=1, col=1)
        ...
        fig.add_shape(type="line",
                      x0=idx, x1=idx + pd.Timedelta(volume_profile_timeframe),
                      y0=poc_price, y1=poc_price,
                      line=dict(color="blue", width=1, dash="dash"),
                      row=1, col=1)
        fig.add_annotation(
            x=idx + pd.Timedelta(volume_profile_timeframe) / 2,
            y=poc_price,
            text=f"POC\n{poc_price:.2f}",
            showarrow=False,
            yshift=10,
...
def main():
    ...
    volume_profile_bins = st.sidebar.slider("Volume Profile Bins", 5, 50, 20)
    volume_profile_tf = st.sidebar.selectbox("Volume Profile Timeframe", ["1D", "1W", "1M"], index=1)
    ...
    with col1:
        ...
        if not df.empty:
            fig = create_chart(df, f"Nifty {interval}min",
                               ...
                               volume_profile_timeframe=volume_profile_tf)
